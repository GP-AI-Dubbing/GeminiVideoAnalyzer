  ...
  static async saveVideo(video: VideoToSave): Promise<void> {
    try {
      ...
      if (testSize > this.MAX_STORAGE_SIZE) {
        console.log(
          `🗜️ Storage limit exceeded (${this.formatBytes(testSize)} > ${this.formatBytes(this.MAX_STORAGE_SIZE)}). Cleaning up...`
        );

        // Remove oldest videos until we have enough space
        const sortedVideos = videos.sort((a, b) => new Date(a.uploadedAt).getTime() - new Date(b.uploadedAt).getTime());
